...
CONTAINS
   ...
   SUBROUTINE dom_init(cdstr)
      ...
      IF(lwp) THEN         ! Ocean domain Parameters (control print)
         ...
         CASE( 8 )   ;   WRITE(numout,*) '         (i.e. equatorial symmetric)'
         CASE DEFAULT
...
908   IF( ios >  0 )   CALL ctl_nam ( ios , 'namnc4 in configuration namelist', lwp )
   ...
   SUBROUTINE cfg_write
      ...
      ENDIF
      !
      IF( .NOT.ln_read_cfg ) THEN   ! analytical configuration: add depth informations for checking
         CALL iom_rstput( 0, 0, inum, 'gdept_0'   , gdept_0  , ktype = jp_r8 )
         CALL iom_rstput( 0, 0, inum, 'gdepw_0'   , gdepw_0  , ktype = jp_r8 )
         CALL iom_rstput( 0, 0, inum, 'ht_0'      , ht_0     , ktype = jp_r8 )
         CALL iom_rstput( 0, 0, inum, 'hu_0'      , hu_0     , ktype = jp_r8 )
         CALL iom_rstput( 0, 0, inum, 'hv_0'      , hv_0     , ktype = jp_r8 )
!!rc         CALL iom_rstput( 0, 0, inum, 'hf_0'      , hf_0     , ktype = jp_r8 )
      ENDIF
      !
