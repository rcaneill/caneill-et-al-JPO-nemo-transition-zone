...
if __name__ == '__main__':
    with sns.axes_style("ticks"):
        ...
        fig, ax = plt.subplots(4, 1, figsize=(5, 5), gridspec_kw={"height_ratios": [0.3, 1, 0.3, 0.3]}, sharex=True)
        ...
        ds.sci.where(ds.tmask_surf).cf.isel(X=X).cf.mean('X').cf.plot.contourf(
            x="latitude", y='gdepw_1d', yincrease=False, cmap=cm_sci, vmin=-1, vmax=1, levels=2,
            add_colorbar=False, ax=ax[1]
        )
        ds.mldr10_1.cf.isel(X=X).cf.mean('X').cf.plot.line(
            x="latitude", ax=ax[1]
        )
        ...
        ds.thetao.where(ds.tmask).cf.isel(X=X).cf.mean('X').cf.plot.contour(
            x="latitude", y='gdept_1d', yincrease=False, cmap=cmo.thermal, levels=np.linspace(0,20,21), ax=ax[1]
        )
        ...
        ds.ztstar.cf.plot.line(
            x="latitude", ax=ax[2], color='gray'
        )
