...
if __name__ == '__main__':
    with sns.axes_style("ticks"):
        ...
        fig, axes = plt.subplots(N, 2, figsize=(pc39, 5/2*N), sharex=True, sharey=True)
        if N == 1:
            axes=np.array([axes])
        ...
        if 'month' in ds.dims:
            for i, t in enumerate(ds.month):
                for j, x in enumerate(ds.glamt):
                    sci_plot = plot_sec(ds.sel(month=t, glamt=x), cm_sci, axe=axes[i,j])
        else:
            # yearly averaged data
            for j, x in enumerate(ds.glamt):
                sci_plot = plot_sec(ds.sel(glamt=x), cm_sci, axe=axes[0,j])

        cb = fig.colorbar(sci_plot, ax=list(axes.flatten()), location='bottom', fraction=0.05)
