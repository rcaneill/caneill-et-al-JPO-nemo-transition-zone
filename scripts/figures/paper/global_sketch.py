...
def plot_sci_bezier(axe):
    ...
    path_data = np.array([
        ...
        (400.28646999,166.47676917), (400.50326999,154.58939614), (400.50326999,122.06260362),
        (399.84028334,123.98463823), (384.26537706,164.30952923), (360.32899099,140.57862315),
        (343.8170314,124.20841289), (308.62384562,43.74657091), (296.71208592,56.73808791),
        (287.25889949,67.04816366), (304.25603239,168.13546246), (306.95547233,183.8603554),
        (309.60603226,199.30050168), (310.12445891,196.64411508), (310.11695225,204.34724822),
        (313.1602855,210.21987474), (396.16572343,199.19924835), (400.39006999,199.12410169),
        (400.39006999,199.12410169)
    ])
    path_data[:,0] /= 10
    path_data[:,0] += 20
    ...
    path_data = np.array([
        ...
        (394.6503768,135.75424994), (391.72213687,150.35928957), (374.49719064,150.98631623),
        (349.16429794,151.90848954), (330.64708507,110.68723323), (318.47992537,119.17657169),
        (313.62296549,122.56539694), (312.88011218,154.24626281), (313.76212549,174.53567564),
        (314.15096548,183.48024875), (315.26457879,196.22870176), (315.29349878,199.10400836),
        (315.35389878,205.11382154), (332.09608503,198.34524837), (354.34612447,198.98387503),
        (369.49365743,199.41866168), (387.14348365,198.5472217), (400.11428333,198.56000837),
        (400.11428333,198.56003504)
    ])
    path_data[:,0] /= 10
    path_data[:,0] += 20
    ...
    path_data[:,1] += 2000
    verts = path_data
    codes = [Path.MOVETO, Path.LINETO] + [Path.CURVE4]*(path_data.shape[0]-3) + [Path.CLOSEPOLY]
    path = mpath.Path(verts, codes)
...
def curly_arrow(x, axe, up=True, n=5, col='gray', linew=.8, width=0.1):
...
if __name__ == '__main__':
    ...
    fig, ax = plt.subplots(3, 1, figsize=(pc39, 4), gridspec_kw={"height_ratios": [0.3, 0.08, 1]}, sharex=True)
    ax = [ax[0],ax[2],ax[1]]
    ...
    ax[0].text(60, f_b_salt[-1], u'$F_b^S$', size="x-large", color='C0')
    ax[0].text(60, f_b_theta[-1]-0.2, u'$F_b^\Theta$', size="x-large", color='C1')
    ax[0].text(40, -1.3, u'$F_b$', size="x-large", color='C2')
    ...
    l = 4
    for (x, up) in zip(
            [20+(x1-20)/l, 20+(x1-20)/l*(l-1), x1+(x2-x1)/l, x1+(x2-x1)/l*(l-1), x2+(60-x2)/l, x2+(60-x2)/l*(l-1)],
            [False, False, True, True, False, False]
    ):
        curly_arrow(x, up=up, n=3, axe=ax[2], col='C2')
    for (x, i) in zip(
        [(20+x1)/2, (x1+x2)/2, (x2+60)/2],
        '><>'
    ):
    ...
    xlim = ax[0].get_xlim()
    ylim = ax[0].get_ylim()
    ax[0].fill_between(xlim, [ylim[1]]*2, 0, color=ax[0].get_xgridlines()[0].get_color(), zorder=0)
    ax[0].set_yticks([-1,.8],minor=True)
    ...
    # Upper plot phi_b
    ax[2].text(x2-.5, .8, r'$\varphi_b$', size='large')

    # Intermediate water creation
    ax[1].annotate(
        "",
        xy=(40,1500), xycoords='data',
    ...
    ax[1].annotate(
        "",
        xy=(50,100), xycoords='data',
        xytext=(40, 100), textcoords='data',
        arrowprops=dict(
            facecolor="w",
            edgecolor='k',
            shrinkB=5,
