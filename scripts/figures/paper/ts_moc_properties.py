...
if __name__ == '__main__':
    ...
    ds.plot.scatter("lat_buoyancy_front", "moc_density_strength", ax=ax[2], marker="v")
    ds.plot.scatter("lat_buoyancy_front", "moc_depth_strength", ax=ax[2], marker="o")

    ax[0].set_title("a) Thermal properties")
    ax[1].set_title("b) Haline properties")
    ax[1].set_title("c) MOC strength")
    ax[1].set_ylim(35.7, 36.1)

    ax[0].set_ylabel(r"Temperature [$^\circ$C]")
    ax[1].set_ylabel(r"Salinity [g$\,$kg$^{-1}$]")
    ax[2].set_ylabel(r"[Sv]")
    ax[0].set_xlabel("")
    ax[1].set_xlabel("")
