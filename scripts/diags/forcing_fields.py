...
zts_eq = 25.0  # [deg C] Temperature at the equator
zts_n = 0.0  # [deg C] Temperature in the north
...
# EMP
zconv = 1.0 / (86400.0)  # convertion factor: 1 mm/day => 1/(3600*24) mm/s
...
zphi01 = 0.0  # [deg North]
zphi02 = 20.0  # [deg North]
zphi03 = 50.0  # [deg North]
z1_d1 = 1.0 / 8.0  # [1 / deg North]
zd2 = 30.0  # [deg North]
z1_d2 = 1.0 / zd2  # [1 / deg North]
zd3 = 40.0  # [deg North]
z1_d3 = 1.0 / zd3  # [1 / deg North]
z1_s = 1.0 / 10.0  # streching of the tanh function (i.e. smoothness of the filter)
...
if __name__ == "__main__":
    ...
    ds["zsstar"] = 37.12 * np.exp(-ds.gphit**2 / 260.0**2) - 1.1 * np.exp(
        -ds.gphit**2 / 7.5**2
    )
    ...
    if ds.ln_ann_cyc:
        ...
        zcos_sais2 = 0 * ds.day
        zcos_sais1 = 0 * ds.day
    ds["ztstar"] = (zts_eq - (zts_n + zcos_sais2 * zdeltaT)) * np.cos(
        (np.pi * ds.gphit) * z1_2L
    ) ** 2 + (zts_n + zcos_sais2 * zdeltaT)
    ds["qsolar_analytic"] = 230.0 * np.cos(
        np.pi * (ds.gphit - 23.5 * zcos_sais1) / (0.9 * 180)
    )
    ...
    tstar = (zts_eq - (zts_n + zcos_sais2 * zdeltaT)) * np.cos(
        (np.pi * gphit) * z1_2L
    ) ** 2 + (zts_n + zcos_sais2 * zdeltaT)
    sstar = 37.12 * np.exp(-(gphit**2) / 260.0**2) - 1.1 * np.exp(
        -(gphit**2) / 7.5**2
    )
